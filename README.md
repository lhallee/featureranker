...
Worked run-throughs with plots, each regenerated by a script in
[examples/](examples/):

- [Breast cancer classification](docs/examples/breast_cancer.md): named
  morphology features, full ensemble, consensus and disagreement.
- [Diabetes regression](docs/examples/diabetes.md): exact L1 path
  breakpoints on clinical features.
- [ModernBERT sentiment](docs/examples/modernbert_sentiment.md): 1,536
  unnamed pooled transformer dimensions ranked directly from a numpy
  matrix; a handful of dimensions carry the signal.
